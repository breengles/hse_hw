...
class Buffer(object):
    def __init__(self, obs_channels, max_size, device="cpu"):
        self.max_size = max_size
        self.device = device

        self.state = torch.zeros(size=(max_size, obs_channels, 11, 11), device=device, dtype=torch.float)
        ...
        self.next_state = torch.zeros(size=(max_size, obs_channels, 11, 11), device=device, dtype=torch.float)
