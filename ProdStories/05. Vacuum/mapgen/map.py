...
TILES = {TileKind.OCCUPIED: "█", TileKind.FREE: ".", TileKind.UNKNOWN: "×", TileKind.AGENT: "O", TileKind.EXPLORED: "_"}
...
class Map:
    def __init__(
        self, tiles: List[List[TileKind]], explored_area: List[List[bool]] = None, trajectory: List[List[bool]] = None
    ):
        self.tiles = np.array(tiles).astype(int)
        self._explored_area = (
            np.array(explored_area) if explored_area is not None else np.zeros_like(self.tiles, dtype=bool)
        )
    ...
    def render(self, agent: Agent):
        ...
        frame = 0.7 * frame + 0.3 * np.tile(
            (1 - self._explored_area * (self.tiles != TileKind.UNKNOWN)) * 200, (3, 1, 1)
        ).transpose((1, 2, 0))
