...
size_long = {
    ...
    "pc_b": (clen, b.size),
    "pc_d": (clen, d.size),
    "pc_ab": (clen, a.size, b.size),
    "pc_abd": (clen, a.size, b.size, d.size),
    "pb_a": (blen, a.size),
    "pb_d": (blen, d.size),
    "pb_ad": (blen, a.size, d.size),
}
size_short = {
    "pa": (alen,),
    "pb": (blen,),
...
def test_distribution(f, model, arg_long, arg_short, size_long, size_short):
    ...
    if var.shape != (size_long[0],):
        raise Exception(
            "Function %s for model %d returns distribution support of the wrong size: %s instead of %s."
